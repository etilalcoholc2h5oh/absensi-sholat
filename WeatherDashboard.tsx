...
type WeatherLocation = { name: string; latitude: number; longitude: number; country?: string };
type WeatherData = {
...
const DEFAULT_LOCATION: WeatherLocation = { name: 'Boyolali', latitude: -7.536065, longitude: 110.596245, country: 'Indonesia' };
...
export const WeatherDashboard: React.FC = () => {
  ...
  const loadWeather = useCallback(async (nextLocation: WeatherLocation) => {
    ...
    try { setWeather(await fetchWeather(nextLocation)); }
    catch (err) { setError(err instanceof Error ? err.message : 'Terjadi kesalahan saat memuat cuaca.'); }
  ...
  const search = async (event: React.FormEvent) => {
    ...
    try { setResults(await findLocation(query.trim())); setError(''); }
    catch (err) { setError(err instanceof Error ? err.message : 'Pencarian lokasi gagal.'); }
  };

  const useCurrentLocation = () => {
    if (!navigator.geolocation) { setError('Browser tidak mendukung lokasi.'); return; }
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      const next = { name: 'Lokasi Anda', latitude: coords.latitude, longitude: coords.longitude };
      setLocation(next); setResults([]);
  ...
  return (
    <section className="weather-shell max-w-7xl mx-auto mt-5" aria-label="Dashboard cuaca">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div><p className="text-sm font-bold text-emerald-700 uppercase tracking-wide">Informasi cuaca</p><h2 className="text-2xl font-extrabold">Pantau cuaca hari ini</h2></div>
      ...
      {isOpen && <>
        ...
        {!loading && weather && <div className="weather-card p-5 sm:p-6">
          ...
          <p className="text-xs text-slate-500 mt-4">Sumber data: Open-Meteo · Zona waktu {weather.timezone}</p>
        </div>}
      </>}
    </section>
  );
